...
@router.post("/knowledge/groups", status_code=fastapi.status.HTTP_201_CREATED, response_model=api_schemas.KnowledgeGroupResponse)
async def create_group(group: api_schemas.CreateKnowledgeGroupRequest, service: km_service.KnowledgeManagementService = fastapi.Depends(dependencies.get_knowledge_management_service)):
    """
...
@router.get("/knowledge/groups/{group_id}", response_model=api_schemas.KnowledgeGroupResponse)
async def get_group(group_id: str, service: km_service.KnowledgeManagementService = fastapi.Depends(dependencies.get_knowledge_management_service)):
    """
...
async def list_group_snapshots(
    group_id: str,
    service: snapshot_service.SnapshotService = fastapi.Depends(snapshot_dependencies.get_snapshot_service)
):
    """
...
@router.post("/knowledge/groups/{group_id}/ingest", status_code=fastapi.status.HTTP_202_ACCEPTED)
async def ingest_group(
        group_id: str,
        km_service: km_service.KnowledgeManagementService = fastapi.Depends(dependencies.get_knowledge_management_service),
        ingestion_service: ingestion_service.IngestionService = fastapi.Depends(dependencies.get_ingestion_service)
    ):
    """
...
@router.patch("/knowledge/groups/{group_id}/sources", response_model=api_schemas.KnowledgeGroupResponse)
async def add_source(
    group_id: str,
    source_data: api_schemas.KnowledgeSource,
    service: km_service.KnowledgeManagementService = fastapi.Depends(dependencies.get_knowledge_management_service)
):
    """
