...
async def get_snapshot(
    snapshot_id: str,
    service: service.SnapshotService = fastapi.Depends(dependencies.get_snapshot_service)
):
    """
...
@router.post("/snapshots/query", response_model=list[api_schemas.KnowledgeVectorResultResponse])
async def query_snapshot(
    request: api_schemas.QuerySnapshotRequest,
    knowledge_service: km_service.KnowledgeManagementService = fastapi.Depends(km_dependencies.get_knowledge_management_service),
    snp_service: service.SnapshotService = fastapi.Depends(dependencies.get_snapshot_service)
):
    """
...
@router.patch("/snapshots/{snapshot_id}/activate", status_code=fastapi.status.HTTP_200_OK)
async def activate_snapshot(
    snapshot_id: str,
    snapshot_service: service.SnapshotService = fastapi.Depends(dependencies.get_snapshot_service),
    knowledge_service: km_service.KnowledgeManagementService = fastapi.Depends(km_dependencies.get_knowledge_management_service)
):
    """
